...
/// POST /config is a no-op in iframe mode (all editing happens in the iframe's
/// own save handler), but we still verify the token so a stale/forwarded call
/// can't ping us anonymously.
...
pub async fn delete_config(
    ...
    // Clean up EventSub subscriptions + cache if no other role link references
    // this broadcaster anymore (shared with the iframe disconnect action).
    if let Some(broadcaster_id) = broadcaster_id {
        crate::routes::broadcaster::cleanup_broadcaster_if_orphaned(&state, &broadcaster_id).await;
    }
